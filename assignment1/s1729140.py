...
def avg_type_length(corpus, list_of_files):
    ...
    # Get a list of all tokens in the corpus
    tokens = list(get_corpus_tokens(corpus, list_of_files))

    # Construct a list that contains the token lengths for each DISTINCT token in the document
    tokens_set = set(tokens)
    distinct_token_lengths = list()
    for token in tokens_set:
        distinct_token_lengths.append(len(token))
    

    # Return the average type length of the document
    avg = 0
    s = 0
    for lengths in distinct_token_lengths:
        s = s + lengths
    
    avg = s/ len(distinct_token_lengths)
    return avg
...
def open_question_1():
    ...
    return inspect.cleandoc("""The average type length is greater for twitter data because tweeets tend to 
contain a lot of stopwords and also nonalphanumeric characters. Twitter data is more noisy and therefore a tokenizer will consider all that noise as different tokens """)[0:500]
...
def train_LM(corpus):
    ...
    lm = LgramModel(2,corpus_tokens, pad_left=True, pad_right= True, estimator=None)
...
def tweet_ent(file_name,bigram_model):
    ...
    for tweet in list_of_tweets:
        for token in tweet:
            if(not(token.isalpha()) or not(token.lower())):
                tweet.remove(token)
    
    cleaned_list_of_tweets = list()
    ...
    for tweet in cleaned_list_of_tweets:
        word_entropy = bigram_model.entropy(tweet, pad_left=True, pad_right= True, perItem=True)
        tuples_list.append((word_entropy,tweet))
...
def tweet_filter(list_of_tweets_and_entropies):
    ...
    list_of_ascii_tweets_and_entropies = list()

    for tweet_entropy in list_of_tweets_and_entropies:
        for tweet in tweet_entropy[1]:
            if(all(ord(c) < 128 for c in tweet)):
                list_of_ascii_tweets_and_entropies.append(tweet_entropy)

    list_of_entropies = list()
    # Extract a list of just the entropy values
    for tweet_entropy_ascii in list_of_ascii_tweets_and_entropies:
        list_of_entropies.append(tweet_entropy_ascii[0]) 
    ...
    list_of_not_English_tweets_and_entropies= list()
